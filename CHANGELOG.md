...
## Quick Chart: recognize the logged-in user, consolidate the two surfaces, add a subscriber PDF export (branch `fix/quick-chart-logged-in-experience`) — 2026-07-11

**Trigger**: two related defects for LOGGED-IN users. (1) Clicking "Quick Chart" in the in-app toolbar (`app-nav.tsx`) opens the public `/chart`, which treated a paying user as anonymous — "Log in" pill, a persistent "Sign up to build your galaxy" footer, and no offer of their own chart. (2) Two inconsistent quick-chart surfaces: the in-app Quick Check bubble (`quick-check-modal.tsx`) was bare (name + Month/Day/Year only) while the public `/chart` form is robust (precision tiers, exact time, geocoded city, mode). The paying user got the WORSE tool.

**Phase 0 — diagnosis (read-only, reported before building).**
- **Why `/chart` didn't recognize the session**: `app/chart/page.tsx` never checked auth at the page level, and `QuickChartShell` hardcoded the anonymous funnel chrome for everyone. Auth is trivially available client-side (`createSupabaseBrowserClient` + `auth.getUser` + a `profiles`/`people` read) — the exact pattern `TrialBanner`, `SaveToGalaxyButton`, and `/chart/compare`'s Person-A prefill already use. `SaveToGalaxyButton` *already* saved directly for logged-in users; only the surrounding chrome/copy treated them as a stranger.
- **The two surfaces**: Quick Check modal = name + date selects, `precision:"date"` hardcoded, no time/location/tiers. Public `/chart(/compare)` = full `<BirthFields>` + mode + romantic/platonic focus. Consolidation reuses `<BirthFields>`, `buildBirthInput`, `computeNatalChart`/`computeSynastry`, `whatTheyNeed`/`sortAspectsForFocus`, `isMinorForSafety`, `CHART_ENGINE_VERSION`.
- **PDF capability**: none in the repo (no `jspdf`/`puppeteer`/`html2canvas`/`react-pdf`). **DECISION — lightest approach: browser print-to-PDF** (`window.print()` + a dedicated `@media print` block), no new dependency, no server route, `next.config.mjs`/`.npmrc`/Vercel untouched (§2).

**DECISION — the free share link is the funnel; the PDF is the perk.** The public web share link stays FREE (acquisition hook, un-paywalled). "Download as PDF" is the subscriber-only perk, gated on the REAL entitlement (`@galaxia/core hasAccess` over `subscription_status`/`trial_ends_at` — active/lifetime or a live trial), never a hardcoded flag. Anonymous visitors never see the button.

**Phase 1 — recognize the logged-in user on `/chart`.** New `lib/use-viewer.ts` client hook resolves `{ userId, subscriptionStatus, trialEndsAt, isSubscriber, selfInput, selfChart }` once, and both `/chart` and `/chart/compare` consume it (compare's bespoke getUser-prefill was folded into it). `QuickChartShell` is now auth-aware: for a recognized user the "Log in" pill → "← My constellation" and the "Sign up to build your galaxy" footer → "← Back to my constellation" (`/app`); **for anonymous visitors and during the loading window the chrome is byte-for-byte the old funnel** — nothing fires without a real session. `/chart` (single) now offers "✦ Use my birth data" to pre-fill from the user's self-person, mirroring the existing Person-A prefill on `/chart/compare`.

**Phase 2 — consolidate the Quick Check modal.** `quick-check-modal.tsx` internals were replaced with the SAME robust flow as public: the shared `<BirthFields>` (exact/date/year tiers, optional time, geocoded city, honest hedging) plus the romantic/platonic focus that reweights which real aspects surface (`sortAspectsForFocus`/`whatTheyNeed`) and a "where it flows and catches" aspect list — feature parity with `/chart/compare`, still a one-tap modal. A year-only person now yields an honest "no full synastry" hedge instead of guessed aspects (§12). The save path preserves `isMinorForSafety` (the age backstop is the ONLY minor protection here, §9) and now persists the full birth data (time/place/coords/tz), not just the date.

**Phase 3 — PDF export (subscriber perk).** New `components/chart-pdf-export.tsx`: a "Download as PDF" button (rendered only when `viewer.isSubscriber`) that calls `window.print()`, plus a clean, on-brand print document portaled into `<body>` and revealed by a `@media print` block in `globals.css` (`print-color-adjust: exact` so the cosmic wheel panel and gold accents survive the print pipeline on a premium cream sheet). It renders wheel + Big Three + full placements, reusing `ChartWheel`, `interpretPlacement`/`interpretRising`, and the derived `houseSystemLabelForChart`. **No fabrication (§12)**: uncertain placements are hedged exactly as on screen, the wheel is omitted with no house cusps, and the house-system label is derived from the chart, never asserted.

**Verified**: `@galaxia/web` `tsc --noEmit` and `next build` pass. Anonymous `/chart` chrome is unchanged; logged-in `/chart` swaps CTAs, offers the self pre-fill, and (for subscribers) exposes the PDF; the Quick Check modal has parity with the public compare flow.
...
**Phase 0 — inventory (read-only, before any change)**:
- **Sections in the raw string, in order**: nav (brand + 5 anchor links + CTA pill, hides all non-CTA links below 860px with **no menu to reach them from at all**), hero (giant poetic h1 + email-free single-CTA form + a `#graph` canvas with 7 named relationship nodes), "the shift" (why), "how it works" (3 steps: See/Understand/Tend, each with a mock card), generational layer (cohort mock + callout), Vela (one example bubble exchange), trust grid (3 cards), FAQ (5 items, accordion via class toggle — the 5th item claimed **"iOS and Android are in development,"** a native-app claim contradicting this task's explicit decision), pricing (2 cards + a 5-item included-list), close (headline + CTA + a **second** email-capture "Notify me" box with `#email2`/`#ok2` — a live waitlist box on an already-launched product), footer.
- **CSS**: a full duplicate `:root` token set (byte-identical to `globals.css`'s, just redefined), a duplicate Google Fonts CDN `@import` (redundant — `next/font/google` already self-hosts Fraunces/Inter site-wide), and ~230 lines of section-specific rules with no equivalent elsewhere.
- **JS handlers**: an ambient starfield canvas + a `#graph` relationship-node canvas (both autonomous drift animation, `prefers-reduced-motion`-aware), a page-wide `IntersectionObserver` driving `.reveal` scroll-fade-ins, FAQ toggle-on-click, and a waitlist-email submit handler for both the hero (already removed in a prior PR) and close-section boxes.
- **Reusable pieces confirmed**: `CosmicBackground` (exact port of the same `.aura`/`.milkyway`/stars/`.grain`/`.vignette` layers — already fixed for overflow and twinkle speed in prior PRs), `.glass-card`/`.eyebrow`/`.lede`/`.btn-primary`/`.container` (already the landing's own classes, just under different names), `AppNav`'s hamburger+drawer CSS (`.app-nav-links`/`.app-nav-trigger-btn`/`.app-nav-drawer`/`.app-nav-drawer-link` — already generic, not in-app-specific), `InitialAvatar`, and the real app's own mockup-source classes (`.sign-chip*`, `.pl-row`/`.glyph-sq`, `.bubble*`/`.chat-thread`, `.compat-word`). Also found, unused anywhere: a pre-built `ConstellationHero` component and a `WaitlistForm` component (both dead/only used by an unrelated legacy `GetApp` component on `/account`) — noted, not touched; out of scope.

**Phase 1 — JSX conversion, verified for parity before restructuring**: every section moved into a real component under `apps/web/components/marketing/*` (`marketing-nav`, `hero`, `hero-graph`, `why-section`, `features-section`, `edge-section`, `vela-example-section`, `trust-section`, `pricing-section`, `faq-section`, `close-section`, `site-footer`, `reveal-observer`). `dangerouslySetInnerHTML` and the injected `<script>` are gone entirely. The `#graph` canvas animation (nodes, curved gradient links, travelling pulse dots, autonomous drift) was ported line-for-line into `hero-graph.tsx` rather than swapped for the unused `ConstellationHero` (visually different — straight links, no pulses, different palette — swapping would have been an undisclosed redesign, not a conversion). The ambient starfield is now `<CosmicBackground/>` directly (provably pixel-identical — it was already "ported precisely" from this same landing per its own doc comment, and its twinkle speed already matches this page's post-fix script). Verified parity with Playwright against the running page: no horizontal overflow at 320/375/390/1440px, zero console/page errors, and the nav/trust/FAQ/pricing/footer sections (unchanged content per the restructure spec) render with the same copy and same interactive behavior (accordion toggle, hamburger).

**Bug found and fixed during parity verification (not present as a *visible* bug before, but real and worth fixing while converting)**: the page-wide `.reveal` + `IntersectionObserver` fade-in — copied faithfully from the original script — raced with initial paint often enough that the hero's own headline and CTA (the most important content on the page, already in the initial viewport with nothing to "scroll into") sometimes rendered stuck at `opacity:0` with no later trigger to reveal them, since there is no scroll event to fire the observer for already-visible content. Reproduced consistently via repeated headless loads. Fixed by giving the hero `.fade-in` (a CSS-only `@keyframes` that plays on mount, already used elsewhere in-app) instead of `.reveal` — below-the-fold sections keep the scroll-triggered `.reveal` pattern, where a user has to scroll first regardless, giving the observer time to attach.

**Phase 2 — restructured to the specified flow**: Hero (poetic line shrunk to an italic kicker, was the full `clamp(3rem,8.4vw,6.6rem)` headline that filled the mobile viewport; a plain-language H1 — "Understand the people you love, through their real charts." — now carries the actual claim; single CTA; a "See how it works ↓" scroll anchor) → The Why (unchanged copy) → Core features, renamed **Add → Understand → Care** (was See/Understand/Tend; the real first move for a new user is adding someone, not "seeing" a chart that doesn't exist yet) → The Edge (the generational layer, reframed as the differentiator — new eyebrow/heading, same real feature and cohort mock) → a Vela example (the exact specified Q&A about a Taurus-Moon/Capricorn-Sun daughter and homework, replacing the old "16 and shuts me out" example — chosen because it visibly shows Vela **refusing** to claim a house/Rising placement it can't support from a missing birth time, then giving real chart-grounded guidance; labeled "An example of how Vela responds") → Trust grid (3 cards, retitled to the specified exact phrasing: "Your notes are yours," "Family-safe: no AI chat with kids," "Real astronomical data, never AI-guessed" — all three verified true of the shipped product, not aspirational: owner-scoped RLS on notes, the age-aware minor-safety gate from the previous PR, and `@galaxia/astro`'s astronomical computation per ENGINEERING.md §8/§12) → Pricing (unchanged $9.99/mo · $89/yr cards; the "no per-person cap / your grandmother shouldn't cost extra" line moved out of the buried 5-item included-list and into its own prominent gold caption directly under the two cards) → FAQ (same 5 questions; the platform-question answer rewritten from *"iOS and Android are in development... your account works across all platforms when the mobile apps launch"* — a native-app claim this task's decisions explicitly forbid — to *"Available now on the web — no downloads, nothing to install. Sign up and you're in."*) → Footer CTA (single final "Start 14 days free"; the second "Notify me" waitlist box and its "Be among the first" line are gone — the product is live, not upcoming).

**Mockups now use the real app's own classes, not marketing-only lookalikes**: the three feature-step cards and the generational cohort card render with `.sign-chip`, `.glyph-sq`/`.pl-body`/`.pl-desc`, `.bubble`/`.chat-thread`, `.compat-word`, and `InitialAvatar` — the literal classes/components `/app/person/[id]`, `/app/groups`, and `/app/vela` render with — not a separate hand-rolled visual approximation. Every mockup carries a small "Illustrative example" caption; none of it is real account data.

**Mobile nav now actually works**: replaced `.nav-links a:not(.nav-cta){display:none}` (every link but the CTA pill simply vanished below 860px, with nothing to open them from) with a new `MarketingNav` component reusing `AppNav`'s exact hamburger+drawer pattern and CSS classes (`.app-nav-trigger-btn`/`.app-nav-drawer`/`.app-nav-drawer-link` — already generic, shared by both navs now) instead of building a second implementation.

**Mobile-first, verified, not just claimed**: `.container` provides the same 32px/22px gutter as every other page (no new "raw HTML page has different margins" class of bug); every grid that stacks a text column next to a mock/visual gets `min-width:0` on both children (CSS Grid's default `min-width:auto` was the exact cause of a prior, separate 320px overflow bug elsewhere on this same page — designed against here from the start, not discovered after the fact). On mobile, the hero's graphic no longer renders `order:-1` (before all text) — it now follows the text, so the headline, subheadline, and CTA are visible without scrolling on a 390px phone, directly serving "make it clear within seconds," which a decorative animation ahead of any copy worked against.

**Verified**: `tsc --noEmit` and `next build` pass (home route dropped from 12.4 kB to 4.62 kB — no more inline HTML string + script payload). Playwright against both the dev server and a production `next start` build: zero horizontal overflow at 320/375/390/1440px (`scrollWidth === clientWidth` at every width), zero console/page errors, hero content confirmed visible (`opacity:1`) on every load, hamburger opens a real drawer with all 7 links + CTA, drawer closes on navigation, FAQ accordion opens/closes, and the last FAQ answer confirmed to read the corrected "Available now on the web" copy with no native-app claim.
