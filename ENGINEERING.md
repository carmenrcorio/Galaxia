...
- Two jobs, `scan` then `push` (`needs: scan`), plus a third `letter` job (`needs: scan`, runs in parallel with `push`) for the Sunday constellation letter. Daily, plus `workflow_dispatch` for an on-demand run from the Actions tab. The letter route no-ops unless the owner's local weekday is Sunday, so one UTC daily trigger still covers every IANA Sunday (UTC Sunday morning is already Sunday from UTC eastward; Monday UTC morning is still Sunday evening in the Americas). The ledger key is owner-local Sunday (`week_of`), so an owner is never emailed twice.
