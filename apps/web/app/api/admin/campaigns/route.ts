...
import { createCampaign } from "../../../../lib/admin/emails";
import { validateCampaignCopy, parseParagraphs, type EmailCopy } from "../../../../lib/email-copy";
...
export async function POST(request: NextRequest) {
  ...
  let campaign;
  try {
    campaign = await createCampaign(serviceRoleClient, { name: body.name, audience: body.audience, copy }, guard.user.id);
  } catch (err) {
    const message = err instanceof Error ? err.message : "Couldn't create the campaign.";
  ...
  try {
    ...
    return NextResponse.json(
      { error: `Campaign created, but the audit log write failed: ${message}. Please tell an engineer.` },
      { status: 500 }
    );
  }
