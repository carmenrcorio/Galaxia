 ...
 *   onto a static canvas (no orbit). Partner is a tight binary at the core (not a guide).
...
function paintNebulaBand(
  ...
  ctx.shadowBlur = band.width * 5 * dpr;
  ctx.shadowColor = band.glow;
  ctx.strokeStyle = band.core;
  ctx.lineWidth = band.width * 1.8 * dpr;
  ctx.beginPath();
  ...
  ctx.shadowBlur = band.width * 2 * dpr;
  ctx.shadowColor = band.core;
  ctx.strokeStyle = band.core;
  ctx.lineWidth = band.width * 0.7 * dpr;
  ctx.beginPath();
  ctx.ellipse(cx, cy, radX, radY, 0, 0, Math.PI * 2);
  ctx.stroke();
  ctx.restore();
...
function drawRingStardust(
  ...
  const prng = (n: number) =>
    ((Math.sin(n * 127.1 + seed * 311.7) * 43758.5453) % 1 + 1) % 1;
  ctx.save();
  ctx.fillStyle = color;
  for (let i = 0; i < count; i++) {
    const angle = prng(i) * Math.PI * 2;
    const spread = (prng(i + count) - 0.5) * 12 * dpr;
    const px = cx + Math.cos(angle) * (radX + spread);
    const py = cy + Math.sin(angle) * (radY + spread);
    const size = (prng(i + count * 2) * 1.2 + 0.3) * dpr;
    ctx.globalAlpha = prng(i + count * 3) * 0.35 + 0.05;
    ctx.beginPath();
...
export default function AppHomePage() {
  ...
  /* Guide rings live on one static canvas under the motion canvas.
  ...
  useEffect(() => {
    ...
    if (!canvas || !atmCanvas || !ringsCanvas) return;
    ...
    const paintRingLayers = () => {
      ...
      sizeRingCanvas(ringsCanvas);
      const ringCtx = ringsCanvas.getContext("2d");
      if (!ringCtx) return;
      if (key === ringBakeKey) return;
      ringBakeKey = key;
      ringCtx.setTransform(1, 0, 0, 1, 0, 0);
      ringCtx.clearRect(0, 0, ringsCanvas.width, ringsCanvas.height);
    ...
    const paintFrame = (opts?: { scheduleRaf?: boolean; exportSettled?: boolean }) => {
      ...
      /* Guide rings live on the static rings canvas, not here. */
    ...
    galaxyCaptureRef.current = async () => {
      ...
      try {
        ...
        if (showRingsRef.current) {
          ...
          const cssW = W();
          const cssH = H();
          paintGuideRingsOnto(rctx, cssW, cssH, DPR, GALAXY_GUIDE_RINGS);
    ...
    return () => {
      ...
      paintRingLayersRef.current = null;
      cancelAnimationFrame(raf);
