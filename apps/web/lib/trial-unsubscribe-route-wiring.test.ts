...
/**
 * Source-level guards for the no-login trial-email unsubscribe route
 * (apps/web/app/api/unsubscribe/route.ts) — the CAN-SPAM-critical piece.
 * Reads source directly for the same server-only-import reason as the
 * nudge unsubscribe wiring tests.
 */

const REPO_ROOT = join(__dirname, "..", "..", "..");
const ROUTE_PATH = "apps/web/app/api/unsubscribe/route.ts";
const MIGRATION_PATH = "supabase/migrations/20260916040503_trial_emails_opted_out.sql";
...
function readRoute(): string {
  return read(ROUTE_PATH);
}

describe("trial unsubscribe route — requires no session, no CRON_SECRET (a lapsed user has neither)", () => {
  const src = readRoute();
...
describe("trial unsubscribe route — token-based, scoped by the unique column, never a raw user id", () => {
  const src = readRoute();
  ...
  it("updates profiles filtered by unsubscribe_token, never by id/email directly", () => {
    expect(src).toMatch(/\.update\(\{\s*trial_emails_opted_out:\s*true\s*\}\)/);
    expect(src).toMatch(/\.eq\("unsubscribe_token",\s*token\)/);
    expect(src).not.toMatch(/\.eq\("id",\s*token\)/);
  });

  it("only ever flips the trial opt-out column — never any other profiles column", () => {
    const updateCalls = [...src.matchAll(/\.update\(\{([^}]*)\}\)/g)].map((m) => m[1].trim());
    for (const call of updateCalls) {
      expect(call).toBe("trial_emails_opted_out: true");
    }
    expect(src).not.toContain("daily_nudge_emails_enabled");
    expect(src).not.toContain("weekly_constellation_letter_enabled");
  });
});

describe("trial unsubscribe route — GET shows a page, POST is a blank one-click ack (RFC 8058)", () => {
  const src = readRoute();
...
describe("trial unsubscribe route — idempotent and never leaks token validity", () => {
  const src = readRoute();
...
describe("trial_emails_opted_out migration — dedicated column, service-role only", () => {
  const src = read(MIGRATION_PATH);

  it("adds trial_emails_opted_out default false, not an enabled flag", () => {
    expect(src).toMatch(/trial_emails_opted_out boolean not null default false/);
  });

  it("never grants the column to authenticated or anon", () => {
    expect(src).not.toMatch(/grant (insert|update)/i);
  });
});
