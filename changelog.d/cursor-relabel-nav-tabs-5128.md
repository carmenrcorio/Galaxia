...
`[CHANGED]` **Marketing nav, footer, app-nav free-chart item, and teaser CTAs.** Visible labels only. Hrefs, H1s, metadata titles, JSON-LD names, and the sitemap are unchanged. `/why-galaxia` reads How it works, `/generations` Your people, `/meet-vela` Ask Vela, `/chart` Free chart. Pricing, Blog, Log in, and the signup CTA are untouched. `/for-work` is not added: no new pages in this branch.
...
`[DECISION]` **Tab order is unchanged.** Grouping and `/for-work` (item F6) wait for a later branch. Homepage teaser H3s stay Why Galaxia / Generations / Meet Vela so those page names remain on `/`.
