-- Cross-user isolation re-test for notes.theme on vela_pin rows.
-- Project: eigfvribtntbxyjutsma
-- Run in Supabase SQL editor as postgres (Dashboard -> SQL).
-- Creates a temporary auth user B, probes a SELECT of notes including theme,
-- then rolls back. Does not change the "notes owner all" policy.
...
DECLARE
  owner_a uuid;
  owner_b uuid := 'bbbbbbbb-cccc-4ddd-8eee-ffffffffffff';
  notes_as_a bigint;
  notes_as_b bigint;
  themes_as_b bigint;
BEGIN
  ...
  VALUES (
    ...
    'authenticated',
    'authenticated',
    'vela-pin-theme-rls-probe-b@galaxia.invalid',
  ...
  SELECT count(*) INTO themes_as_b FROM public.notes WHERE theme IS NOT NULL;

  RAISE NOTICE 'notes_owned_by_A=% (expect > 0)', notes_as_a;
  RAISE NOTICE 'notes_visible_to_B=% (expect 0)', notes_as_b;
  RAISE NOTICE 'themes_visible_to_B=% (expect 0)', themes_as_b;

  IF notes_as_b <> 0 THEN
    RAISE EXCEPTION 'ISOLATION_FAILED notes_visible_to_B=%', notes_as_b;
  END IF;
  IF themes_as_b <> 0 THEN
    RAISE EXCEPTION 'ISOLATION_FAILED themes_visible_to_B=%', themes_as_b;
  END IF;

  RAISE EXCEPTION 'ISOLATION_OK notes_visible_to_B=0 themes_visible_to_B=0: rolling back probe user';
END $$;
