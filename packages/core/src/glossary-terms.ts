/**
 * Shared static glossary. `/glossary` and in-app GlossaryTerm popovers
 * both read this list. Terms were extracted from published `public.posts`
 * bodies (status = 'published', 10 rows) by counting distinct-post
 * mentions; the list is curated to technique and placement words that
 * show up in at least two posts, plus the remaining major aspect
 * (sextile), outer planets (Uranus, Neptune), and orb (chart-view
 * tooltip). Saturn return and retrograde did not appear in the corpus
 * and are omitted.
 *
...
export const GLOSSARY_TERMS: readonly GlossaryTerm[] = [
  ...
  // FOUNDER-REVIEW: orb definition for chart and compare tooltips.
  {
    id: "orb",
    term: "Orb",
    postCount: 0,
...
export type AspectGlossarySlug = (typeof ASPECT_GLOSSARY_SLUGS)[number];

export function getGlossaryTerm(id: string): GlossaryTerm | undefined {
  return GLOSSARY_TERMS.find((item) => item.id === id);
}

export function aspectGlossarySlug(type: string): AspectGlossarySlug | undefined {
  const key = type.trim().toLowerCase();
  return (ASPECT_GLOSSARY_SLUGS as readonly string[]).includes(key)
    ? (key as AspectGlossarySlug)
    : undefined;
}

export function glossaryPreview(definition: string, maxSentences = 2): string {
  const matches = definition.match(/[^.!?]+[.!?](?:\s|$)/g);
  if (!matches || matches.length === 0) return definition.trim();
  return matches.slice(0, maxSentences).join("").replace(/\s+/g, " ").trim();
}

// FOUNDER-REVIEW
export const GLOSSARY_SEE_FULL_DEFINITION = "See full definition";
