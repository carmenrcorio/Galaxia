-- Internal-linking pass on the seeded synastry post
-- (20260909011620_blog_posts_schema_and_content.sql). That migration has
-- already been applied, so per ENGINEERING.md §2 ("supabase/migrations/*
-- once applied — never edit an applied one") this ships as an UPDATE in a
-- new migration rather than a hand-edit of the original INSERT.
--
-- Three contextual links added to the markdown body, matching the internal
-- linking pass across the standalone marketing pages:
--   1. "What a chart cannot tell you" section -> /meet-vela, right where the
--      post is telling the reader a chart alone can't substitute for real
--      guidance — Vela is that guidance.
--   2. "Reading yours" section -> /generations, on the exact sentence that
--      already mentions "the generational layer".
--   3. Closing CTA paragraph -> /chart/compare ("Try a free synastry
--      chart"), the natural next step once someone has read the whole post.
--
-- read_time_minutes is recomputed by hand here (apps/web/lib/blog.ts
-- computeReadTimeMinutes: words / 225wpm, rounded up) to match what the
-- admin editor would have stored had this been saved through /admin/posts
-- instead of a migration. The added sentences push the count from 1312 to
-- 1363 words, which crosses the ceil(words/225) boundary from 6 to 7.
update public.posts
set
