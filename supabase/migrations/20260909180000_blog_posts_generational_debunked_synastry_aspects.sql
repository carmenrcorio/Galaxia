-- Three new blog posts (content growth: generational-astrology moat,
-- sun-sign skepticism / debunked category, and a synastry-aspects deep
-- dive), plus a reciprocal internal link added to the existing
-- synastry-chart-meaning post — same seam as the migrated post in
-- 20260909011620_blog_posts_schema_and_content.sql, just via INSERT/UPDATE
-- against the now-live `posts` table instead of table creation. Every post
-- here is written and published the same way an admin would through
-- /admin/posts (lib/admin/posts.ts) — full title/dek/body/category/status,
-- no draft rows left behind.
--
-- Category "debunked" fills that category page's first real post
-- (previously zero posts, see BLOG_CATEGORIES' emptyNote in
-- apps/web/lib/blog.ts) at /blog/debunked; the post itself still lives at
-- its own top-level URL (/sun-sign-not-personality), same as every other
-- post — /blog/debunked is the category listing, not a post route.
...
(
  ...
  'Nobody''s Astrology App Has Your Grandmother In It',
  'Every astrology app knows your Mercury sign. None know your grandmother''s. Galaxia charts everyone in your life — even the ones you''ve lost.',
  'guides',
  $POST_BODY$Every astrology app on your phone knows your Mercury placement. Ask it about your grandmother's, and it has nothing.
...
14 days free — galaxiamea.com$POST_BODY$,
  'published',
  6,
  'The Galaxia Team',
...
(
  ...
  'Sun-sign horoscopes miss 97% of your chart. Here''s what they leave out, why compatibility tables lie, and what actually works.',
  'debunked',
  $POST_BODY$You read your horoscope. It says something about being bold in conversations this week. You are, at this exact moment, hiding in a bathroom at a party because someone asked you a direct question. You are, allegedly, a Leo.
...
(
  ...
  '7 Synastry Aspects That Predict How Relationships Feel',
  'Venus conjunct Mars, Moon square Saturn, and 5 more synastry aspects that shape how a relationship actually feels. What your chart lines mean.',
  'guides',
  $POST_BODY$You finally pulled a synastry chart with someone — a partner, a sibling, the friend you've been close with for a decade — and now you're staring at a wheel with lines running everywhere between two sets of planets, and no idea which ones actually matter.
...
-- Reciprocal link: the new synastry-aspects-explained post links back to
-- this one from its intro ("If you're new to synastry, start with our
-- intro guide"); this adds the return link from the original post's body,
-- placed right after the "shared name for it" paragraph in the "Reading
-- the catches" section — a natural jumping-off point for a reader who
-- wants the deeper aspect-by-aspect breakdown. `updated_at` bumps via the
-- explicit set below (no trigger on this table maintains it automatically).
update public.posts
set body = $POST_BODY$Most writing about synastry is aimed at someone standing at the beginning of something, trying to decide whether to walk in. Score out of ten. Green flags, red flags, verdict.
