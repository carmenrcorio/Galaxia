...
-- 20260909120000_synastry_post_internal_links.sql was written before the
-- em-dash purge (20260911161000_posts_rewrite_em_dash.sql) and was applied
-- after it, so it restored U+2014 into the synastry-chart-meaning body
-- while adding the three internal links. This UPDATE rewrites only those
-- reintroduced dashes in the live body. Links, titles, dek, and
-- read_time_minutes are unchanged. Applied migrations are never edited
-- (ENGINEERING.md §2); this is a new file. Do not re-run either prior
-- migration.
